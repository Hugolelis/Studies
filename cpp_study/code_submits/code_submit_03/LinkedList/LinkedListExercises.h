...
void LinkedList<T>::insertOrdered(const T& newData) {
    ...
    if(head_ == nullptr) {  // lista vazia
        head_ = tail_ = newNode;
    } 
    else if (newData < head_->data) {  // inserir no início
        newNode->next = head_;
        head_->prev = newNode;
        head_ = newNode;
    } 
        ...
        Node* current = head_;
        // Andar até achar o local de inserção
        while (current->next != nullptr && current->next->data < newData) {
            current = current->next;
        }

        // Inserir depois do current
        newNode->next = current->next;
        newNode->prev = current;
        if (current->next != nullptr) {
            current->next->prev = newNode;
        } else {
            tail_ = newNode; // atualiza tail se inserir no fim
        }
