...
static void treeFactory(GenericTree<int>& tree) {
    ...
    // Limpa a árvore se já tiver conteúdo
    ...
    auto node15 = root->addChild(15);

    // Adiciona filhos de 8
    auto node16 = node8->addChild(16);
    auto node23 = node8->addChild(23);

    // Adiciona filho de 16
    node16->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
    ...
    // Fila para processamento nível a nível
    ...
    while (!q.empty()) {
        ...
        // Adiciona o dado do nó atual
        results.push_back(current->data);

        // Adiciona os filhos do nó atual à fila, da esquerda para a direita
        for (TreeNode* child : current->childrenPtrs) {
            if (child != nullptr) {
                q.push(child);
            }
        }
